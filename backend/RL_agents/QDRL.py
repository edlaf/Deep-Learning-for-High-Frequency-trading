    ...
    def train(self, visu=True, visu_graph = True, nb_episode = 1000, window_size = 10, frequency_action = 2, comparaison = False):
        ...
        for episode in pbar:
            ...
            while not done:
                ...
                actions = torch.LongTensor([action]).unsqueeze(0).to(self.device)
                rewards = torch.FloatTensor([reward]).unsqueeze(0).to(self.device)
                next_states = torch.FloatTensor(next_state).unsqueeze(0).to(self.device)
                dones = torch.FloatTensor([done]).unsqueeze(0).to(self.device)
